...
| 알고리즘 | 기능 |
|----------|------|
| 📐 동적 가중치 정규화 | 사용자 정의 가중치에 대한 자동 부동소수점 정규화 — 할당 불균형 방지 |
| 🎯 시나리오 적응형 튜닝 | 에이전트가 경기 수준/팀 속성/대진 특성을 자동 식별 — 가중치를 동적으로 조정 |
| 🔇 다차원 노이즈 제거 및 신뢰도 | 무효 데이터 필터링, 신뢰도 수준 등급 지정, 이상치 제거 |
| 📊 소셜 감성 정량화 | 감성 극성 점수 (-10~10), 여론 압력 지수, 집중도 평가 |
| ⚖️ 크로스 팩터 밸런스 보정 | 다요소 상호 균형 및 보완적 보정 — 단일 데이터에 의한 오류 유도 방지 |
